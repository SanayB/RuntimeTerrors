...
def get_dashboard(q: str = "", riskLevel: str = "", classification: str = "", skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """
    Returns a paginated list of scan results for the admin dashboard.
    ...
    if q:
        ...
        query = query.filter(
            or_(
                func.lower(ScanResult.domain).like(like_query),
                func.lower(ScanResult.employee_email).like(like_query),
                func.lower(ScanResult.department).like(like_query)
            )
        )

    if riskLevel:
        query = query.filter(ScanResult.risk_level == riskLevel.upper())

    if classification:
        query = query.filter(ScanResult.classification == classification.lower())

    results = query.order_by(ScanResult.scanned_at.desc()).offset(skip).limit(limit).all()
